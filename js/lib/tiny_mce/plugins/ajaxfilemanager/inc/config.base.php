...
//error_reporting(E_ALL ^ E_NOTICE);	
    ...
    $configyii=dirname(__FILE__).'../../../../../../../protected/config/main.php';
    require_once($yii);
    if(!Yii::app())
        Yii::createWebApplication($configyii);  
                        
    if(Yii::app()->user->checkAccess('filemanager')){
        $base_dir='../../../../../content/';
    	$td_dir=$base_dir.'media';
        if(!is_dir($td_dir))
            mkdir($td_dir);
    }
     
    
	define('CONFIG_SYS_DEFAULT_PATH', $td_dir); //accept relative path only
	define('CONFIG_SYS_ROOT_PATH', $base_dir);	//accept relative path only
	...
	define('CONFIG_UPLOAD_VALID_EXTS', 'gif,jpg,png,bmp,tif,zip,rar,gz,tar,mov,mpg,avi,asf,mpeg,wmv,aif,aiff,wav,mp3,swf,ppt,rtf,doc,pdf,txt,xls,xlsx');// // 
	...
	define('CONFIG_LANG_DEFAULT', (CONFIG_QUERY_STRING_ENABLE && !empty($_GET['language']) && file_exists(DIR_LANG . secureFileName($_GET['language']) . '.php')?secureFileName($_GET['language']):'ru')); //change it to be your language file base name, such en
?>